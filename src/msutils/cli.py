...
@click.option("-v", "--verbose", is_flag=True,
              help="Also list scans, antennas and columns in full.")
@click.option("--level", type=click.Choice(["meta", "full", "data"]),
...
@click.option("--json-stdout", is_flag=True,
              help="Print JSON to stdout instead of the text report.")
@click.option("--format", "fmt", type=click.Choice(["MSv2", "MSv4"]),
              default=None, help="Force the format instead of auto-detecting.")
@click.option("--engine", type=click.Choice(["casacore", "zarr", "xradio",
                                             "xarray-ms"]),
              default=None,
...
def summary(ms, outfile, quiet):
    ...
    click.echo("warning: 'msutils summary' is deprecated; use 'msutils info'.",
               err=True)
...
@click.option("--spw", "spws", multiple=True,
              help="Spectral window id/name to include (repeatable).")
@click.option("--scan", "scans", multiple=True, type=int,
              help="Scan number to include (repeatable).")
...
@click.option("--force", is_flag=True,
              help="Allow removing columns the MSv2 standard requires.")
...
def _selection_options(command):
    ...
    for option in reversed([
            click.option("--field", "fields", multiple=True,
                         help="Field id/name to keep (repeatable)."),
            click.option("--spw", "spws", multiple=True,
                         help="Spectral window id/name to keep (repeatable)."),
            click.option("--scan", "scans", multiple=True, type=int,
                         help="Scan number to keep (repeatable)."),
            click.option("--antenna", "antennas", multiple=True,
                         help="Antenna id/name to keep (repeatable)."),
    ]):
...
@_selection_options
@click.option("--taql", "taql_where", default=None,
...
def subset(ms, outms, fields, spws, scans, antennas, taql_where, overwrite):
    ...
    _subset(ms, outms, fields=list(fields) or None, spws=list(spws) or None,
            scans=list(scans) or None, antennas=list(antennas) or None,
            taql=taql_where, overwrite=overwrite)
    click.echo(outms)
...
@click.argument("ms")
@click.argument("outms")
@click.option("--time-bin", type=float, default=1.0, show_default=True,
              help="Averaging interval in seconds.")
@click.option("--chan-bin", type=int, default=1, show_default=True,
              help="Channels per output channel.")
@_selection_options
@click.option("--datacolumn", default="DATA", show_default=True,
              help="Visibility column to average.")
@click.option("--overwrite", is_flag=True, help="Replace OUTMS if it exists.")
def average(ms, outms, time_bin, chan_bin, fields, spws, scans, antennas,
            datacolumn, overwrite):
    """Time- and channel-average MS into OUTMS (needs the 'average' extra)."""
    from .subset import average as _average

    try:
        _average(ms, outms, time_bin=time_bin, chan_bin=chan_bin,
                 fields=list(fields) or None, spws=list(spws) or None,
                 scans=list(scans) or None, antennas=list(antennas) or None,
...
def du(ms, as_json):
    ...
    usage = _du(ms)
    click.echo(json.dumps(usage.to_dict(), indent=2) if as_json
               else usage.render())
...
def check(ms, as_json):
    ...
    report = _check(ms)
    click.echo(json.dumps(report.to_dict(), indent=2) if as_json
               else report.render())
    if not report.ok:
        raise SystemExit(1)
...
@click.option("--partition", "partitions", multiple=True,
              type=click.Choice(["FIELD_ID", "SCAN_NUMBER", "STATE_ID",
                                 "ANTENNA1"]),
              help="Extra column to partition on (repeatable).")
@click.option("--no-pointing", is_flag=True,
              help="Skip the POINTING subtable (often large and unused).")
