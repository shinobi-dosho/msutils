...
def _table(headers: Sequence[str], rows: Sequence[Sequence[Any]], indent: str = "  ") -> list[str]:
    ...
    out = [
        indent
        + "  ".join(
            h.rjust(widths[i]) if aligns[i] else h.ljust(widths[i]) for i, h in enumerate(headers)
        )
    ]
    ...
    for row in cells:
        out.append(
            indent
            + "  ".join(
                c.rjust(widths[i]) if aligns[i] else c.ljust(widths[i]) for i, c in enumerate(row)
            ).rstrip()
        )
    return out
...
def render(info: MSInfo, verbose: bool = False) -> str:
    ...
    out.append(
        f"  format      : {info.format}{engine}    size: {format_bytes(info.size_bytes)}    rows: {info.nrows:,}"
    )
    out.append(
        "  telescope   : {:<12} observer: {:<16} project: {}".format(
    ...
    if info.start_utc:
        ...
        if (
            verbose
            and obs.time_start is not None
            and (
    ...
    if info.integration_times:
        out.append(
            "  integration : {}".format(", ".join(f"{t:g}s" for t in info.integration_times))
        )
    ...
    if info.flagged_fraction is not None:
        out.append(
            f"  flagged     : {info.flagged_fraction * 100.0:.2f}% of {info.nvisibilities:,} visibilities"
        )
        ...
        _table(
            ...
            [
                [
                    ...
                    s.num_chan,
                    s.frame,
                    format_frequency(s.centre_freq),
                    format_frequency(s.total_bandwidth),
    ...
    if verbose or _nontrivial_ddid(info):
        ...
        out.extend(
            _table(
                ["DDID", "SPW", "Pol"], [[d.id, d.spw_id, d.pol_id] for d in info.data_descriptions]
            )
        )
    ...
    if verbose:
        ...
        out.extend(
            _table(
                ...
                [
                    [
                        s.number,
                        s.field_name or s.field_id,
                        s.start_utc,
                        format_duration(s.duration),
                        ",".join(str(i) for i in s.spw_ids),
                        f"{s.nrows:,}",
                        _shorten(s.intents),
                    ]
                    for s in info.scans
        ...
        out.append("")
        out.append("Columns ({}): {}".format(len(info.columns), ", ".join(info.column_names)))
