...
def to_msv4(msname: str, outpath: str,
    ...
    LOGGER.info("Converting %s -> %s (MSv4, partition_scheme=%s)",
                msname, outpath, scheme or "default")
    ...
    LOGGER.info("Wrote %d MSv4 partition(s) covering %d field(s) and %d SPW(s)",
                len(info.subtables), info.nfields, info.nspws)
    return info
...
def _honour_requested_path(outpath: str) -> None:
    ...
    if os.path.exists(written):
        LOGGER.debug("xradio wrote %s; moving to the requested %s",
                     written, outpath)
